...
client.on('message', message => 
{
	...
	{
		//message.reply("If A code is not 32 characters: Hey that doesn't seem quite right :tired_face: ... are you sure your verification code is 32 characters long? Try again or Contact a Developer :smile:"); 
		...
		//	message.delete(1000); //deletes after 1 second 
			...
			//{
			    if (command === `${config_new.prefix}key`)
			    {
					...
					//let sql = "SELECT * FROM verification WHERE verification_code = \"" + verification + "\"";  
					let steamKeySQL = "SELECT * FROM steam_keys WHERE key_given = 0 LIMIT 1 WHERE discord_username = null";
								...
								//	if (signupIdResults[0].verification_used == 0) //if key has not been used 
									//{
										...
										tempConnection.query(steamKeySQL, function(err, steamKeyResults)
										{
											...
											{
												//let setVerificationActive = "UPDATE verification SET verification_used = 1 WHERE verification_code = \"" + signupIdResults[0].verification_code + "\""; 
												//let setVerificationName = "UPDATE verification SET discord_username = \"" + username + "\" WHERE verification_code = \"" + signupIdResults[0].verification_code + "\"";  
												...
												//console.log(signupIdResults[0]); 
												//console.log(steamKeyResults[0]); 
											}
